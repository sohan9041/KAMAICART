...
import {
  ProductVariant
} from "../Models/productVariantModel.js";
import {
  ProductVariantAttributeValue
} from "../Models/ProductVariantAttributeValueModel.js";
  ...
  try {
    ...
    if (req.files && req.files.length > 0) {
      const imageRecords = req.files.map((file) => ({
        ...
        image_url: `/uploads/products/${file.filename}`, // or use cloud URL if uploading to S3
      }));
    ...
    for (const variantData of variants) {
      ...
      const variant = await ProductVariant.create(
        { ...variantInfo, product_id: product.id },
        { transaction: t }
      );
    ...
    await t.commit();
    return apiResponse.successResponseWithData(res, "Product with variants & images created", product);
  ...
  try {
    ...
    if (Array.isArray(variants) && variants.length > 0) {
      for (const variantData of variants) {
        ...
        if (variantId) {
          variant = await ProductVariant.findOne({ where: { id: variantId, product_id: id }, transaction: t });
          if (variant) {
            ...
            if (sku && sku !== variant.sku) {
              ...
              if (existingSku) {
                throw new Error(`SKU "${sku}" already exists. Please choose a different SKU.`);
              }
            }
          ...
          const existingSku = await ProductVariant.findOne({ where: { sku }, transaction: t });
          if (existingSku) {
            throw new Error(`SKU "${sku}" already exists. Please choose a different SKU.`);
          }
          variant = await ProductVariant.create({ ...variantInfo, sku, product_id: id }, { transaction: t });
        }

        // ✅ Update attributes only if provided
        if (Array.isArray(attributes) && attributes.length > 0) {
          await ProductVariantAttributeValue.destroy({ where: { variant_id: variant.id }, transaction: t });
    ...
    if (err.name === "SequelizeUniqueConstraintError") {
      return apiResponse.ErrorResponse(res, `SKU already exists. Please choose a different SKU.`);
    }
